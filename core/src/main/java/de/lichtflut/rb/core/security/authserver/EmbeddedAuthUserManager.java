...
/**
 * <p>
 * There are several possible implementations of an authentications service,
 * e.g. an external LDAP service, or an Open ID server. In the most simple
 * scenario the RB application itself provides the authentication services.
 ...
 */
public class EmbeddedAuthUserManager implements AuthenticationService,
		UserManager {
	...
	private final Logger logger = LoggerFactory
			.getLogger(EmbeddedAuthUserManager.class);
	 ...
	 *            The domain manager.
	 */
	public EmbeddedAuthUserManager(ArastrejuGate gate,
			EmbeddedAuthDomainManager domainManager) {
	...
	public RBUser login(LoginData loginData) throws LoginException {
		...
		if (id == null) {
			throw new LoginException(ErrorCodes.LOGIN_INVALID_DATA,
					"No username given");
		}
		...
		if (user == null) {
			throw new LoginException(ErrorCodes.LOGIN_USER_NOT_FOUND,
	...
	public RBUser loginByToken(String token) {
		...
		try {
			...
			logger.error("Failed to login by token " + Arrays.toString(fields),
					e);
		}
	 ...
	 */
	@Override
	public void registerUser(RBUser user, String credential, String domainName)
		...
		SNOPS.associate(userNode, Aras.IDENTIFIED_BY,
				new SNText(user.getEmail()), Aras.IDENT);
		SNOPS.associate(userNode, Aras.IDENTIFIED_BY, new SNText(user
				.getQualifiedName().toURI()), Aras.IDENT);
		if (user.getUsername() != null) {
			SNOPS.assure(userNode, RBSystem.HAS_USERNAME, user.getUsername());
			SNOPS.associate(userNode, Aras.IDENTIFIED_BY,
					new SNText(user.getUsername()), Aras.IDENT);
			SNOPS.associate(userNode, RBSystem.HAS_USERNAME,
					new SNText(user.getUsername()), Aras.IDENT);
		}
		if (user.getEmail() != null) {
			SNOPS.associate(userNode, RBSystem.HAS_EMAIL,
					new SNText(user.getEmail()), Aras.IDENT);
		}
	...
	public void updateUser(RBUser updated) throws RBAuthException {
		final ResourceNode attachedUser = masterGate.startConversation()
				.findResource(updated.getQualifiedName());
		...
		if (updated.getUsername() != null
				&& !Infra.equals(existing.getUsername(), updated.getUsername())) {
		...
		if (updated.getUsername() != null) {
			SNOPS.assure(attachedUser, RBSystem.HAS_USERNAME,
					updated.getUsername());
		} else {
	 ...
	 */
	@Override
	public void verifyPassword(RBUser user, String currentPassword)
			throws RBAuthException {
		ResourceNode arasUser = findUserNode(user.getEmail());
		final String checkCredential = SNOPS.string(SNOPS.singleObject(
				arasUser, Aras.HAS_CREDENTIAL));
	...
	public Set<String> getUserPermissions(RBUser user, String domain) {
		return authorization
				.getUserPermissions(user.getQualifiedName(), domain);
	}
	 ...
	 */
	@Override
	public void setUserRoles(RBUser user, String domain, List<String> roles)
			throws RBAuthException {
	 ...
	 */
	@Override
	public void removeAllUserRoles(RBUser user, String domain)
			throws RBAuthException {
		authorization.setUserRoles(user, user.getDomesticDomain(),
				Collections.<String> emptyList());
	}

	// ----------------------------------------------------

	protected void verifyPassword(final ResourceNode user, String password)
		...
		if (!credential.applies(singleObject(user, Aras.HAS_CREDENTIAL))) {
			throw new LoginException(
					ErrorCodes.LOGIN_USER_CREDENTIAL_NOT_MATCH,
					"Wrong credential");
		}
	}
	...
	protected ResourceNode findUserNode(final String id) {
		...
		if (result.size() > 1) {
			...
			throw new IllegalStateException("More than on user with name '"
					+ id + "' found.");
	...
	private void setLastLogin(final ResourceNode user) {
		SNOPS.assure(user, RBSystem.HAS_LAST_LOGIN, new SNTimeSpec(new Date(),
				TimeMask.TIMESTAMP));
	}
	...
	private String getSalt(ResourceNode user) {
		...
		final String credential = SNOPS.string(SNOPS.singleObject(user,
				Aras.HAS_CREDENTIAL));
